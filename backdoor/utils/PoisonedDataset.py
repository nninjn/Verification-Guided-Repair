...
class PoisonedMNIST(MNIST):
    ...
    def __init__(
        ...
        self.pattern = torch.tensor(np.full((5, 5), 255), dtype=torch.uint8)
...
class BlendMNIST(MNIST):
    ...
    def __init__(
        ...
        super(BlendMNIST, self).__init__(root, train=train, transform=transform, download=download,
                                    target_transform=target_transform)
...
if __name__ == "__main__":
    ...
    # aa = iter(aa)
    # print(next(aa))
    trg = imread('/home/ailab/majianan/abLocal/backdoor/utils/trigger1.jpg')
    trg = np.array(trg)
    print(trg.shape)
