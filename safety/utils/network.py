...
class FNN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.only_logits:
            ...
            return [x, x1, x2, x3, x4, x5, x6, output]

    def split(self, layer):
        if layer == 4:
            return nn.Sequential(self.dense5, nn.ReLU(), self.dense6, nn.ReLU(), self.out)
        elif layer == 5:
            return nn.Sequential(self.dense6, nn.ReLU(), self.out)
        elif layer == 6:
            return nn.Sequential(self.out)
        else:
            raise ValueError("Split error")


class FNN_ver(nn.Module):

    def __init__(self):
        super(FNN_ver, self).__init__()
